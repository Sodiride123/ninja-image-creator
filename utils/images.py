...
def edit_image(
    ...
    Args:
        ...
        mask_path:  Optional path to a mask PNG. Transparent (alpha=0) areas
                    will be regenerated. If None, the entire image is edited.
        model:      Model alias or full ID. Default: "gpt-image".
        size:       Output size. One of: "1024x1024", "1024x1536", "1536x1024".
        output:     Output file path for the edited image.
        timeout:    Request timeout in seconds.

    Returns:
        Path to the saved edited image file.
    ...
    if r.status_code != 200:
        error = r.json().get("error", {}).get("message", r.text[:300])
        raise RuntimeError(f"Image edit failed ({r.status_code}): {error}")

    resp_data = r.json()
    if "data" not in resp_data or len(resp_data["data"]) == 0:
        raise RuntimeError(f"No image data in edit response: {resp_data}")
    ...
    if item.get("url"):
        img_r = requests.get(item["url"], timeout=60)
        if img_r.status_code != 200:
            raise RuntimeError(f"Failed to download edited image: {img_r.status_code}")
        Path(output).write_bytes(img_r.content)
        return output
    ...
    raise RuntimeError(f"Edit response has no url or b64_json: {item}")
